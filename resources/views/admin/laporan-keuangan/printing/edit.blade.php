...
@section('content')
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form action="{{ url('admin/order/printing/update/' . $transaksi->id_pesanan_printing) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <input type="text" name="nama_dokument_lama" value="{{ $transaksi->dokumen }}" hidden class="form-control">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('warna') is-invalid @enderror" name="warna" id="warna" required>
                        ...
                        <option  value="Color" {{ old('warna') == 'Color' || $transaksi->warna == 'Color' ? 'selected' : '' }}>Color</option>
                        <option value="Grayscale" {{ old('warna') == 'Grayscale' || $transaksi->warna == 'Grayscale' ? 'selected' : '' }}>Grayscale</option>
                    </select>
                    @error('warna')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('kertas') is-invalid @enderror" name="kertas" id="kertas" required>
                        ...
                        <option value="Tidak" {{ old('kertas') == 'Tidak' || $transaksi->kertas == 'Tidak' ? 'selected' : '' }}>Tidak</option>
                        <option value="Ya" {{ old('kertas') == 'Ya' || $transaksi->kertas == 'Ya' ? 'selected' : '' }}>Ya</option>
                    </select>
                    @error('kertas')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('jumlah') is-invalid @enderror" id="jumlah"
                        name="jumlah" placeholder="Masukkan jumlah lembar dokumen" value="{{ old('jumlah', $transaksi->jumlah) }}" required>
                    @error('jumlah')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
